...
func (s *testSQLDigestSuite) TestNormalize(c *C) {
	...
	for _, test := range tests {
		normalized := parser2.Normalize(test.input)
		digest := parser2.DigestNormalized(normalized)
		c.Assert(normalized, Equals, test.expect)

		normalized2, digest2 := parser2.NormalizeDigest(test.input)
...
func (s *testSQLDigestSuite) TestNormalizeDigest(c *C) {
	...
	for _, test := range tests {
		normalized, digest := parser2.NormalizeDigest(test.sql)
		c.Assert(normalized, Equals, test.normalized)
		c.Assert(digest.String(), Equals, test.digest)

		normalized = parser2.Normalize(test.sql)
		digest = parser2.DigestNormalized(normalized)
		c.Assert(normalized, Equals, test.normalized)
		c.Assert(digest.String(), Equals, test.digest)
	}
